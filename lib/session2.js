...
Session.prototype.createClient = function () {
	...
	this.client.on('error', function (err) {
        ...
        this.client.on('afterCompile', function (obj) {
          if(!obj.success)
            return

          var script = obj.body.script
            , match = script.name.match(/^_couchdb:(.*)\.js$/)
            , name = match && match[1]
          ...
          console.log('afterCompile for %j\n%s', name, util.inspect(obj, false, 20))
          self.parsedScripts([script])
          console.log('Ran parsedScripts: %j', script)
        })
